...
# AutoRound fork of blazux/qwen3.8-Flash-DGX.
#
...
# mmap, failed READs retry/stall). Container must run with --device /dev/infiniband
# --ulimit memlock=-1:-1.
...
RUN gcc -O2 -Wall -shared -fPIC /tmp/ple_rdma.c -libverbs \
      -o ${SP}/libple_rdma.so && rm /tmp/ple_rdma.c
COPY src/vllm_ple_rdma.py ${SP}/vllm_ple_rdma.py
